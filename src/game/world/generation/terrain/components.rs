...
pub fn gen_terrain(world: &VoxelWorld, col: ColPos, fast_noise: &NoisesList) {
    ...
    weird_noise.set_frequency(0.005);

    let mut erosion_noise = FastNoise::new();
    erosion_noise.set_frequency(0.0025);

    let mut continental_noise = FastNoise::new();
    continental_noise.set_frequency(0.002);

    let mut pv_noise = FastNoise::new();
    pv_noise.set_frequency(0.0001);
    ...
    for (dx, dz) in iproduct!(0..CHUNK_S1, 0..CHUNK_S1) {
        ...
        let height = (((temperature_noise.get_noise(offset_x + dx as f32, offset_z + dz as f32)
            + 1.)
            * 0.5)
            ...
            + ((continental_noise.get_noise(offset_x + dx as f32, offset_z + dz as f32) + 1.)
                * 0.5)
        ...
        world.set_yrange(col, (dx, dz), y, 4, Blocks::sand());
...
fn calculate_height(
    ...
    precipitation_noise: &FastNoise,
    offset_x: f32,
    offset_z: f32,
    x: f32,
    z: f32,
) -> f32 {
    let base_height = (((temperature_noise.get_noise(offset_x + x, offset_z + z) + 1.) * 0.5)
        + ((precipitation_noise.get_noise(offset_x + x, offset_z + z) + 1.) * 0.5) * 0.5);

    let base_height =
        base_height * ((continental_noise.get_noise(offset_x + x, offset_z + z) + 1.) * 0.5);

    let base_height =
        base_height * (erosion_noise.get_noise(offset_x + x, offset_z + z) + 1.) * 0.5;
    base_height
}
