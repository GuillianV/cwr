...
use super::noise::resources::{ArcPerlinNoiseMap, PerlinNoiseMap};

pub fn setup_gen_thread(blocks: Res<VoxelWorld>, perlin_noise_map : Res<ArcPerlinNoiseMap>,/* world_rng: Res<WorldRng>, */load_orders: Res<LoadOrders>) {
    let thread_pool = AsyncComputeTaskPool::get();
    let chunks = Arc::clone(&blocks.chunks);
    // let seed_value = world_rng.seed;
    let load_orders = Arc::clone(&load_orders.to_generate);
    let perlin_noise_map = Arc::clone(&perlin_noise_map.map);
